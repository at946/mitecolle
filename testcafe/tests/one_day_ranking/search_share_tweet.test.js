...
test('one day rankingページで、シェア数を選択したとき、そのスライドのシェアツイートを検索するページに遷移すること', async t => {
  await t.expect(page.slide(0).shareCount.getAttribute('href')).eql(`https://twitter.com/search?q=${encodeURIComponent('http://localhost:3000/1')}&f=live`)
  await t.expect(page.slide(0).shareCount.getAttribute('target')).eql('_blank')
  await t.expect(page.slide(9).shareCount.getAttribute('href')).eql(`https://twitter.com/search?q=${encodeURIComponent('http://localhost:3000/10')}&f=live`)

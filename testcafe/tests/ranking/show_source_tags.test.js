...
test('one week rankingページで、スライドのソースとしてSpeaker Deckがあることが表示されること', async (t) => {
  const speaker_deck_tag = await page.source_services_tag.nth(2)
...
test('one week rankingページで、スライドのソースとしることが表示されること', async (t) => {
  const slide_share_tag = await page.source_services_tag.nth(1)
...
test('one week rankingページで、スライドのソースとしてドクセルがあることが表示されること', async (t) => {
  const docswell_tag = await page.source_services_tag.nth(0)

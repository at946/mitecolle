...
const Slide: NextPage<Props> = (props: Props) => {
  return (
    <div data-testid="slide">
      <p>
        <a href={props.url} target="_blank" rel="noopener noreferrer" className="has-text-dark has-text-weight-bold is-size-5" data-testid="slide_title">{props.title}</a>
      </p>
      <p className="mb-4">
        <span className="tag help is-rounded is-info is-light" data-testid="slide_share_count">{props.shareCount} tweets</span>
      </p>
      <div>
        <iframe
          src={props.iframeSrc}
